...
import Archivo.Empresa.LectorDeArchivos;
...
/*public class TestLectorDeArchivos {
	...
	public void ParserYLectorDeArchivos() {
		...
		empresasAParsear = LectorDeArchivos.entregarLista();
		...
		Assert.assertEquals(Parser.aEmpresas(empresasAParsear).get(2).getPeriodos().get(1).getCuentas().get(0).getValor(), 9000);
		
		
		Assert.assertEquals(Parser.aEmpresas(empresasAParsear).get(3).getPeriodos().get(0).getCuentas().get(1).getValor(), 7171);
	}
	
	@Test
	public void ParserYLectorDeArchivosNachito() {
		List<List<String>> empresasAParsear = new ArrayList<List<String>>();
