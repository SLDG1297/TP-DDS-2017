...
public class MetodologiasRepository {
	...
	public static MetodologiasRepository getInstancia() {
		if (instancia == null) {
			...
			instancia.agregarMetodologia(new Metodologia("MayorA_500_En1_Periodo", new CondicionFactory().crearMayorAEnPeriodos(IndicadoresRepository.getInstancia().getIndicadores().get(0), new BigDecimal(500), 1)));
			instancia.agregarMetodologia(new Metodologia("MayorA_20_En1_Periodo", new CondicionFactory().crearMayorAEnPeriodos(IndicadoresRepository.getInstancia().getIndicadores().get(0), new BigDecimal(20), 1)));
			instancia.agregarMetodologia(new Metodologia("MayorA_500_En3Periodo_Cuenta", new CondicionFactory().crearMayorAEnPeriodos(IndicadoresRepository.getInstancia().getIndicadores().get(4), new BigDecimal(500), 3)));
			instancia.agregarMetodologia(new Metodologia("MenorA_500_En4Periodo_Cuenta", new CondicionFactory().crearMenorAEnPeriodos(IndicadoresRepository.getInstancia().getIndicadores().get(4), new BigDecimal(500), 4)));
			instancia.agregarMetodologia(new Metodologia("PromedioMayorA_750_Cuenta", new CondicionFactory().crearPromedioMayorA(IndicadoresRepository.getInstancia().getIndicadores().get(4), new BigDecimal(750))));
			instancia.agregarMetodologia(new Metodologia("SumatoriaMenorA_2000_Cuenta", new CondicionFactory().crearSumatoriaMenorA(IndicadoresRepository.getInstancia().getIndicadores().get(4), new BigDecimal(2000))));
